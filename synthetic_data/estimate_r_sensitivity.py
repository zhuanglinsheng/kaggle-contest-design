...
DATA_PATH = ROOT / "synthetic_data" / "data_90_1000.json"
OUT_PATH = ROOT / "synthetic_data" / "contest_90_posteriors_estimate_r.csv"
...
def fn_invgamma_approx(x: float) -> float:
    return math.atan(0.856 * x) * 2.0 / math.pi


def efforts(y: float, t: float, T: float, theta: float, sigma: float, c_i: float, c_j: float) -> tuple[float, float]:
    if T <= t:
        return 0.0, 0.0
    sigma2 = sigma * sigma
    ...
    gamma_i = fn_gamma(rho_i)
    gamma_j = fn_gamma(rho_j)
    y_sd = sigma * math.sqrt(T - t)
    z = y / y_sd
    rho_z = fn_invgamma_approx(ndtr(z) * (gamma_i + gamma_j) - gamma_j)
    density_y = math.exp(-0.5 * (y / y_sd) ** 2) / (math.sqrt(2.0 * math.pi) * y_sd)
    K = sigma2 / 2.0 * (gamma_i + gamma_j) * (1.0 - rho_z * rho_z)
    return density_y * K * (1.0 + rho_z), density_y * K * (1.0 - rho_z)
...
class SyntheticLikelihood:
    def __init__(self, data: dict):
        ...
        self.Ni = int(data["Ni"])
        self.Nj = int(data["Nj"])
        self.idx_i = np.ceil(np.asarray(data["hat_t_i"], dtype=float)).astype(int) + 1
        self.idx_j = np.ceil(np.asarray(data["hat_t_j"], dtype=float)).astype(int) + 1
        self.events_idx = np.sort(np.concatenate([self.idx_i, self.idx_j]))
        n = self.Ni + self.Nj
    ...
    def paths(self, x: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        tilde[0] = mu_0
        root_lam_sig = math.sqrt(lamb) * sigma
        for i in range(self.N_delta):
            mi, mj = efforts(tilde[i], i * self.delta, self.T, self.theta, sigma, c_i, c_j)
            m_i[i] = mi
            m_j[i] = mj
            kalman_gain = root_lam_sig * (self.hat_y[i] - tilde[i])
            tilde[i + 1] = tilde[i] + (mi - mj + kalman_gain) * self.delta
    ...
    def logpost_x(self, x: np.ndarray) -> float:
        ...
        mean_y = csum_gap[self.events_idx - 1] * self.delta
        cov = sigma * sigma * self.unit_cov_y + self.eye / (self.delta * lamb)
        try:
...
def main() -> None:
    ...
    # simulation value.
    x_start = np.array([1.089, 1.751, 2.801, 1.105, 0.005, 15.0])
