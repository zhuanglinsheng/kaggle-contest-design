...
This approximation preserves the tractable tug-of-war structure of \citet{Ryvkin2022Fight}.
...
\begin{remark}
The approximation is accurate when \(w=\theta/(\sigma^2c)\) is small, or equivalently when the prize-cost ratio \(\theta/c\) is small relative to the innovation variance \(\sigma^2\). 
In this regime, the first-order expansion in \(w\) is most informative.
\end{remark}
...
Contest designers can influence contestants’ effort through the prize amount \(\theta\), contest duration \(T\), and the precision of public feedback \(\lambda\). 
Abstracting from more complex prize structures, we examine whether prize and duration can substitute for each other in eliciting effort and whether more precise feedback changes effort incentives.

Under the terminal-payoff rule based on the filtered mean and the steady-state filtering assumption \(S_t=\bar S\), however, signal precision \(\lambda\) drops out of both the closed-form equilibrium effort policy and total expected effort \(M(\tilde y_t,t)\). 
This neutrality follows from \(\lambda\bar S^2=\sigma^2\). 
It is specific to the maintained assumptions: 
\(\lambda\) may affect equilibrium effort if terminal payoffs depend on posterior winning probabilities or if filtering uncertainty varies over time. 
We therefore focus the following design analysis on the prize amount and contest duration.

Under the symmetric-cost assumption \(c_i=c_j=c\), equation~\eqref{eq-totaleffort-reduced} provides a simple approximation to total expected effort.
To study the prize-duration trade-off, suppose the contest designer has the stylized objective $\beta^{T}M(\tilde{y}_0,0)^\alpha-\theta$, where $0<\alpha<1$ and $0<\beta\leq 1$.
...
Although \(\lambda\) does not affect equilibrium effort under the maintained assumptions, it still affects how contestants update their beliefs from public signals and how much noise enters the observed public leaderboard. 
It therefore remains an estimable parameter in the structural model developed below.
...
% Players
The second criterion requires two highly active, top-ranked contestants who engage in sustained competition during the contest. 
We identify candidate focal contestants using final private leaderboard ranks, beginning from the top of the ranking. 
This choice aligns the two-player approximation with rivalry among contestants who perform best under the contest's final evaluation criterion. 
Private path values do not enter the estimation likelihood, but final private ranks determine the focal pair; the validation exercises are therefore path-wise holdout diagnostics conditional on ex post pair selection.
...
We therefore interpret the two-player model as a structural approximation of the dominant rivalry near the top of contests with concentrated competition, frequent submissions, and overlapping activity windows.
...
Because asynchronous updates may reuse an opponent’s carried-forward score across successive readouts, the resulting public-private discrepancies can be serially dependent. We therefore interpret the benchmark as an approximate measure of signal precision.
...
\right\},
\]
where $\underline\theta=0.01$ thousand USD is the lower prize bound used in the numerical search. For each duration multiplier considered below, direct numerical evaluation confirms that $\theta=\theta^{obs}$ satisfies the effort constraint, so the feasible set is nonempty.
...
Table~\ref{tab-counterfactual-policy} reports the substitution frontier for duration multipliers close to the observed design.
When $a=1$, the counterfactual coincides with the observed design and no prize reduction is possible.
For each positive duration increase considered in Table~\ref{tab-counterfactual-policy}, predicted total effort can be preserved with a lower prize in all 73 contests.
For the contest-specific parameter estimates and local duration changes considered here, predicted total effort is numerically increasing in both prize and duration; the empirical content is the magnitude of the required prize adjustment.
A one percent increase in duration allows a median prize reduction of $0.65\%$, while a ten percent increase allows a median prize reduction of $5.95\%$.
The corresponding mean percentage changes are more negative than the medians, suggesting that a subset of contests permits especially large prize reductions.
We therefore emphasize the medians as the more representative summary.
...
$1.01$ & $73/73$ & $15/73$ & $1/73$ & $1/73$ & $-1.09\%$ & $-0.65\%$ \\
$1.02$ & $73/73$ & $66/73$ & $2/73$ & $1/73$ & $-1.92\%$ & $-1.28\%$ \\
$1.05$ & $73/73$ & $73/73$ & $12/73$ & $2/73$ & $-4.15\%$ & $-3.11\%$ \\
$1.10$ & $73/73$ & $73/73$ & $51/73$ & $9/73$ & $-7.37\%$ & $-5.95\%$ \\
\bottomrule
\addlinespace[0.5ex]
\end{tabularx}
\begin{minipage}{\textwidth}
...
For each contest, we compute the minimum prize $\theta_a\in[0.01,\theta^{obs}]$ thousand USD such that predicted total effort remains weakly above $M(\theta^{obs},T^{obs})$. At each candidate design, total expected effort is evaluated as $M(\tilde y_0,0)=\sigma\sqrt{T}\,q(\tilde y_0/(\sigma\sqrt{T}))$, where $q$ solves the ordinary differential equation in the equilibrium characterization using the contest-specific estimates of $(c_i,c_j,\sigma,\mu_0)$ and the analytical approximation to $\gamma^{-1}$ used in estimation.
...
Across 73 Kaggle contests, these path-wise holdout diagnostics provide evidence on the estimated primitives.
