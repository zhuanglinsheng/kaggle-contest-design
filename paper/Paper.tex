...
\hat{y}_t = \int^t_0\left[m_i(\tilde{y}_t, s) - m_j(\tilde{y}_t, s)\right]ds + \sigma W_{t} + \frac{dB_t}{dt\sqrt{\lambda}}
\end{equation}
where the term $\sigma W_t$ captures the innovation shock, and $dB_t/(dt\sqrt{\lambda})$ represents the signal noise.
...
&\quad\prod_{k=0}^{N-1}\phi\left(\hat{y}_{t_{k+1}}-\hat{y}_{t_k}|\int_{t_k}^{t_{k+1}}m_i(\tilde{y}_s, s) - m_j(\tilde{y}_s, s)ds, \sigma^2(t_{k+1}-t_k)+\frac{2}{\Delta\lambda}\right)
\end{aligned}
\end{equation}
