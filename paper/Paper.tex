...
In the Markov perfect equilibrium, the players’ efforts in state $m_{i(j)}(y, t) :\mathbb{R}\times[0, T)\to\mathbb{R}_+$ are given by
...
\begin{algorithmic}
\STATE \textbf{Input:} 
	$\Delta t$, $T$, $c_i$, $c_j$, $\sigma$, $\lambda$, $r$, $\tau^\star$
...
\STATE Initialize $\ell^{i(j)}=0$, $y_0 = 0$, $\hat{y}_0 = 0$, $\tilde{y}_t=0$
\FOR{$t = 0$ to $T$}
    \STATE $m_{i(j)}(\tilde{y}_t, t)$  $\gets$ (\ref{eq-equilibrium-effort}), $\tau^{i(j)}(t)$ $\gets$ (\ref{eq-model-intensity})
    \STATE $y_{t+\Delta t} \gets$ (\ref{eq-state-dynamics}); $\tilde{y}_{t+\Delta t}$ $\gets$ (\ref{eq-fintered-y-update})
