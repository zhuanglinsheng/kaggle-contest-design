...
\midrule
 ...
 Posterior Std.       & 0.382 & 0.661 & 3.986 & 0.503 & 0.303 & 2.026 \\
...
Next, with the submission times denoted by $\{t_k\}$, we have $y_{t_{k+1}} - y_{t_k} \sim \mathcal{N}\left(\int^{t_{k+1}}_{t_k}m_i(s) - m_j(s)ds, \sigma^2(t_{k+1}-t_k)\right)$ by the dynamic of private leaderboard (\ref{eq-state-dynamics}). 
Then, by (\ref{normal-mle-sigma}), we can estimate the parameters $\sigma$ using MLE: 
\begin{equation*}
\sigma^{\text{MLE}} = \left(\frac{1}{n}\sum_k\frac{\left[y_{t_{k+1}}-y_{t_k}-\int^{t_{k+1}}_{t_k}m_i(s) - m_j(s)ds\right]^2}{t_{k+1}-t_k}\right)^{1/2}
\end{equation*}
