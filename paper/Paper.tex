...
\end{equation}
Once the parameter set $\Theta := \{c_i, c_j, \sigma, \lambda\}$ is specified, the data-generating processes for $y_t$, $\hat{y}_t$, and $\tilde{y}_t$ are fully defined by equations (\ref{eq-state-dynamics}), (\ref{eq-leaderboard-gap}) and (\ref{eq-fintered-y-update}), though their realized trajectories remain stochastic.


\subsection{Bayesian Estimation Framework}

Our objective is to develop a Bayesian framework for inferring the parameter set $\Theta := \{c_i, c_j, \sigma, \lambda\}$, using the data introduced above.
...
1. How to select key players? Top $n$ in final private leaderboard
