...
\end{equation*}
The benchmark therefore uses only submission-triggered leaderboard readouts rather than repeated hourly carry-forward observations. Because asynchronous updates may repeatedly use an opponent's carried-forward score, the discrepancies can be serially dependent; we therefore interpret $\lambda_\ell^{\mathrm{bench}}$ as an approximate signal-precision benchmark.
...
Table~\ref{tab-counterfactual-policy} reports the substitution frontier for duration changes close to the observed design. For each positive duration increase considered, the numerical search identifies a lower prize that preserves predicted total effort in all 73 contests. A 1\% increase in duration allows a median prize reduction of 0.65\%, while a 10\% increase allows a median reduction of 5.95\%. Because the mean reductions exceed the medians, the distribution is right-skewed; we therefore emphasize the medians as the more representative summary.
