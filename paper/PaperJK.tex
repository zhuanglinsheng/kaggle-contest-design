...
Applying the framework to 75 Kaggle contests, we find statistically meaningful but uneven alignment between structurally recovered latent quantities and private-leaderboard benchmarks constructed from excluded post-contest information.
...
Under the approximation in (\ref{eq-totaleffort-reduced}), this objective is tractable for numerical optimization over $(\theta, T)$.
...
Having shown that the structural estimates exhibit meaningful alignment with private-leaderboard-based benchmarks, we next use the model for counterfactual analysis. 
A key advantage of structural estimation is that it permits policy experiments that are not directly observed in the data. 
In the context of online crowdsourcing contests, platform operators can adjust design levers such as prize size, contest length, and feedback informativeness. 
Our estimated model provides a disciplined way to examine how such changes would affect equilibrium effort, submission activity, and expected contest outcomes. 
Accordingly, this subsection turns from validation to counterfactual policy simulations.
These exercises should be interpreted as model-based comparative statics under the maintained two-player, steady-state filtering specification, rather than as definitive policy prescriptions.


\textbf{Winner-Take-All Prize Amount $\theta$.}


\textbf{Contest Duration $T$ and Deadline Effects.}


\textbf{Information Design: Signal Precision $\lambda$}
