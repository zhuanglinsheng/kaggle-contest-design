...
Under the maintained Gaussian discrepancy structure implied by \eqref{eq-leaderboard-gap}, the public-private discrepancy yields the following benchmark maximum likelihood estimator (MLE) for $\lambda_c$.
...
\end{equation*}
Here, $N^c_i$ and $N^c_j$ denote the numbers of submissions by players $i$ and $j$ in contest $c$.
...
The transformation is adopted for tractability and does not guarantee strict normality, especially in extreme contests; empirically, only about half of contests pass the Shapiro-Wilk test.
...
For each contest $c$, we estimate $\hat{\sigma}_c$ structurally using the two players’ public submission records $\{t_k^c\}_{k=1}^{N_i^c+N_j^c}$ and public leaderboard trajectories $\{\hat y_k^c\}_{k=1}^{N_i^c+N_j^c}$.
...
$y^c_{k+1} - y^c_{k} \sim \mathcal{N}\left(\mu^c_k, \sigma^2(t^c_{k+1}-t^c_k)\right)$.
Here, $\mu^c_k = \int^{t^c_{k+1}}_{t^c_k}\left[m^c_i(s) - m^c_j(s)\right]ds$.
...
\end{equation*}
where $\hat\mu^c_k$ is retrieved from Bayesian posterior effort trajectories.
...
meaningful information about the contest environment. We use them to revisit the
design question in Section~\ref{sect-design}, where the designer's objective is
motivated by $\beta^{T}M(\tilde{y}_0,0)^\alpha-\theta$. Because the designer's
true cardinal payoff is difficult to calibrate in the empirical application, we
do not compare objective levels directly. Instead, we ask whether observed
contest designs leave room for \textit{prize-effort Pareto improvements}\footnote{
...
can predicted total effort be increased without raising the prize, or can the prize
be reduced without lowering predicted total effort?

For each contest, we hold fixed the estimated structural primitives and search
for alternative prize-duration designs by jointly optimizing over $(\theta,T)$
within a bounded redesign region. A counterfactual design generates a
prize-effort Pareto improvement if, relative to the observed design, it weakly
lowers prize spending and weakly increases predicted total effort, with at least
one strict improvement. This comparison is defined in the prize-effort outcome
space, rather than as a full welfare comparison across all agents in the
contest. In the implementation, we search over
$\theta\in[0.01,\theta^{obs}]$ and $T\in[0.25T^{obs},3T^{obs}]$. The analysis is
based on the 73 contests with complete observed design inputs and structural
estimates.

Table~\ref{tab-counterfactual-policy} summarizes the results. The model implies
substantial room for improvement along both dimensions. For all 73 contests, the
counterfactual search finds a positive effort-improvement margin: predicted
total effort can be increased without raising the prize. The average
effort-improvement margin is $96.31\%$, with a median of $81.53\%$. The
prize-saving margin is also positive for all 73 contests: the prize can be
reduced while keeping predicted total effort weakly above its observed-design
level. The average no-loss prize-saving margin is $59.65\%$, with a median of
$60.08\%$; in absolute terms, this corresponds to an average prize reduction of
$3.00$ thousand USD.
...
{\footnotesize
\textit{Note:}
This table reports counterfactual redesign results for the 73 contests with complete observed design inputs and structural estimates. For each contest, the observed design is $(\theta^{obs},T^{obs})$, and predicted total effort under this design serves as the baseline. The counterfactual search holds the estimated structural primitives fixed and jointly varies prize size and contest duration over $\theta\in[0.01,\theta^{obs}]$ and $T\in[0.25T^{obs},3T^{obs}]$. The effort-improvement column reports the maximum percentage increase in predicted total effort that can be achieved without increasing the prize above $\theta^{obs}$. The prize-saving column reports the maximum percentage reduction in prize spending that can be achieved while keeping predicted total effort weakly above the observed-design baseline. Prize is measured in thousand USD. The absolute effort change is in model-implied effort units; the absolute prize change is in thousand USD.
}
\end{minipage}
\end{table}

The main takeaway is that, within the maintained two-player structural model,
observed contest designs appear widely improvable. They do not seem to lie on
the model-implied frontier between prize spending and predicted effort. This
does not mean that the observed platform choices are globally suboptimal in a
full welfare sense. Real contests may involve operational constraints, entry
responses, heterogeneous participants, and other objectives outside the model.
Rather, the counterfactual exercise shows that, along the prize and effort
margins captured by the model, observed designs leave substantial room for
improvement.
