...
For validation, we use a holdout design: private-leaderboard path values are excluded from the estimation likelihood and reserved for diagnostic tests, although final private ranks are used only to define the focal rivalry.
...
Although private-leaderboard path values are excluded from estimation, final private ranks are used to select the focal pair of contestants. 
We therefore interpret the results as diagnostic evidence conditional on the selected pair, rather than as validation based on a fully independent sample. 
...
The actual contest is equipped with a submission system in which participants submit solutions over time, with each submission updating both the hidden private-leaderboard output gap and its noisy public-leaderboard readout.
% Signal
...
To retain the tractable tug-of-war structure of \citet{Ryvkin2022Fight}, we specify terminal payoffs based on the filtered mean: player \(i\) receives the prize if \(\tilde y_T>0\), whereas player \(j\) receives it if \(\tilde y_T<0\). This threshold rule replaces a terminal expected payoff based on the posterior winning probability, such as \(\theta\Pr(y_T>0\mid I_T)\).
...
\begin{remark}
The approximation is accurate when $w=\theta/(\sigma^2 c)$ is small, i.e., when uncertainty $\sigma$ is high relative to the prize-cost ratio $\theta/c$. 
This is the regime where the first-order expansion in $w$ is most informative.
\end{remark}
...
Under the perceived-state terminal payoff and the steady-state filtering assumption \(S_t=\bar S\), signal precision \(\lambda\) does not enter the closed-form equilibrium effort policy or total expected effort \(M(\tilde y_t,t)\). 
This neutrality follows from the steady-state relation \(\lambda\bar S^2=\sigma^2\), which causes signal precision to drop out of the effort policy conditional on the perceived state. 
The result is specific to the simplified specification: if terminal payoffs depended on posterior winning probabilities or filtering uncertainty varied over time, \(\lambda\) could affect equilibrium effort.
...
We define the latent contemporaneous gap as $y_t:=x_t^i-x_t^j$ and the displayed public gap as $\hat{y}_t:=\hat{x}_t^i-\hat{x}_t^j$, where each displayed score is carried forward from that contestant's most recent submission.
...
% tilde{y}_t
The theoretical model uses a continuous public signal and therefore admits the continuous-time filtering equation in \eqref{filtered-x}. The empirical leaderboard, by contrast, is observed only at asynchronous submission times. A fully coherent empirical implementation would therefore require a hybrid filter with continuous prediction between submissions and discrete measurement updates when new scores arrive.

To retain the tractable one-dimensional state representation used in the equilibrium analysis, we employ a continuous-update approximation in the empirical implementation. Specifically, we interpolate the most recently observed public-gap process and evolve the perceived gap according to
...
This approximation treats the carried-forward leaderboard gap as a continuous proxy for the unavailable signal path. It may overstate learning during long intervals without submissions, so the resulting estimates should be interpreted as conditional on this continuous-update approximation rather than as estimates from an exact discrete-observation filter.
...
\]
where $\mu_{k\mid k-1}$ and $V_{k\mid k-1}$ are obtained from the prediction step over $(t_{k-1},t_k]$ before the update at $t_k$ is incorporated. The leaderboard likelihood is therefore
...
The estimation likelihood uses only contest-phase submission timing and public leaderboard trajectories. Private-leaderboard paths and score values are excluded from the likelihood and are used to construct post-estimation validation targets. Final private ranks, however, enter the focal-pair selection rule, so the resulting design should be interpreted as path-wise holdout validation rather than complete sample-selection independence.
...
% Players
The second criterion requires two highly active, top-ranked contestants who engage in sustained competition during the contest. We identify candidate focal contestants using final private leaderboard ranks, beginning from the top of the ranking. This choice aligns the two-player approximation with rivalry among contestants who perform best under the contest's final evaluation criterion. Private path values do not enter the estimation likelihood, but final private ranks determine the focal pair; the validation exercises are therefore path-wise holdout diagnostics conditional on ex post pair selection.
To ensure sustained overlapping activity and adequate data for estimation, we impose two conditions: each contestant must submit at least five times, and the pair must satisfy the temporal-overlap criterion used in sample construction.
These criteria help ensure that the focal-pair data contain enough submission activity and temporal overlap for structural estimation.
...
Because submissions are asynchronous, an update by one contestant combines that contestant's new score with the opponent's most recently submitted score. The resulting public gap is therefore a carried-forward leaderboard gap rather than a synchronous measurement of both contestants' contemporaneous latent outputs. Our observation equation treats this gap as an approximate readout of the latent competitive state; this approximation is most credible for focal pairs with frequent submissions and substantial overlap in active periods.
...
Under the maintained approximation that the discrepancy at these readouts follows $\hat{y}_{t_k}^\ell-y_{t_k}^\ell\sim\mathcal N(0,1/(\lambda_\ell h_k^\ell))$, $\forall t_k\in\mathcal E_\ell$, we construct
\begin{equation*}
...
Because contestant submissions are asynchronous, both displayed public and private gaps may combine a newly updated score for one contestant with a carried-forward score for the other. The discrepancy benchmark therefore inherits the carried-forward-gap approximation and should be interpreted as an approximate signal-precision diagnostic rather than as an exact MLE under synchronous observations.

It is important to emphasize that this benchmark relies on the Gaussian discrepancy approximation applied to the transformed scores in \eqref{eq-leaderboard-transform}.
...
We then regress the contest-specific private-leaderboard benchmark $\lambda_\ell^{\text{bench}}$ on the Bayesian structural estimate $\hat\lambda_\ell$ to examine whether a positive relationship holds between the two.
...
Thus, the estimated innovation-risk primitive is positively associated with an approximate private-leaderboard consistency benchmark whose path values are excluded from the likelihood.
...
affect predicted total effort.\footnote{
...
contest duration while holding predicted total effort fixed.
...
=
\min\left\{
\theta\in[\underline\theta,\theta^{obs}]
:
M(\theta,T_a)\geq M_0
\right\},
\]
where $\underline\theta=0.01$ thousand USD is the lower prize bound used in the numerical search. Under the maintained monotonicity of predicted effort in duration, the feasible set is nonempty because $\theta=\theta^{obs}$ satisfies the effort constraint for $a\geq1$.
...
possible. For each positive duration increase considered in Table~\ref{tab-counterfactual-policy}, predicted total effort can be preserved with a lower
...
increase allows a median prize reduction of $11.03\%$. The corresponding means
are substantially larger than the medians, indicating a strongly right-skewed
distribution of substitution rates. We therefore emphasize the medians as the
...
For each contest, we compute the minimum prize $\theta_a\in[0.01,\theta^{obs}]$ thousand USD such that predicted total effort remains weakly above $M(\theta^{obs},T^{obs})$.
...
Conditional on focal pairs selected using final private ranks, we recover latent strategic objects using only contest-phase submission timing and public leaderboard trajectories. We then assess the estimated contest-specific primitives with private-leaderboard path benchmarks excluded from the likelihood.
Across 73 Kaggle contests, these path-wise holdout diagnostics provide disciplined evidence on the estimated primitives.
...
% - leaderboard display
(3) leaderboard scores vary in format, complicating their integration into a unified signal structure; and
(4) the focal pair is selected using final private ranks, so the private-path validation is conditional on ex post pair selection rather than fully independent.
% 2. Kalman-filter
Second, the empirical filtering implementation involves two additional approximations. The estimation method simplifies belief updating by assuming the inference variance quickly converges and remains constant.
A more realistic approach would allow inference precision to evolve over time.
The empirical implementation also approximates asynchronous, submission-triggered leaderboard updates with a continuous-update filtering equation. A fully coherent hybrid filter with discrete measurement updates would better represent stale scores and irregular observation times but would sacrifice much of the tractability of the current framework.
Third, cross-contest comparisons of innovation risk and private-path volatility may retain a shared transformed-score scale component despite the common logit mapping.
% Future
